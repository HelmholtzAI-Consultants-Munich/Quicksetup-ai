...
# and `pip install ML-Pipeline-Template` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "ML-Pipeline-Template", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n ML-Pipeline-Template -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "ML-Pipeline-Template", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "ML-Pipeline-Template", "YOUR_CONSOLE_SCRIPT"]
